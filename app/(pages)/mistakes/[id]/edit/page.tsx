...
export default function EditMistakePage() {
  ...
  useEffect(() => {
    ...
    async function fetchMistake() {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch mistake data')
        }
        const data = await response.json()
        setFormData({
          ...data,
          mistakeDate: new Date(data.mistakeDate).toISOString().split('T')[0],
        })
      } catch (err) {
        setError('Failed to load mistake data. Please try again.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess('Mistake updated successfully!')
        setTimeout(() => {
          router.push('/mistakes')
        }, 1500)
      } else {
        setError(result.error || 'Failed to update mistake log')
      }
    } catch (err) {
        ...
        <Card className="shadow-2xl border-0 bg-white/90 backdrop-blur-lg overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 text-white">
            <CardTitle className="text-2xl font-bold flex items-center">
              ...
              Edit Mistake Details
            </CardTitle>
            <CardDescription className="text-blue-100 text-base">
              Update the information about the mistake to keep the records accurate.
            </CardDescription>
          </CardHeader>
