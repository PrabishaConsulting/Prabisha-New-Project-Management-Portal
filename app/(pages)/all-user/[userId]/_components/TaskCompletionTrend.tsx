...
import { getTaskCompletionTrendData } from "@/actions/user"
...
export function TaskCompletionTrend({ userId,initialData }: TaskCompletionTrendProps) {
  ...
  React.useEffect(() => {
    if (!initialData) {
      const fetchData = async () => {
        setIsLoading(true)
        try {
          const data = await getTaskCompletionTrendData(userId , timeRange)
          setChartData(data)
        } catch (error) {
