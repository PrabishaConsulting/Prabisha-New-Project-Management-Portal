...
export default function TaskTable({ data }: TaskTableProps) {
  ...
  const handleExportToExcel = () => {
    const dataToExport = table.getFilteredRowModel().rows.map(row => ({
        ...
        'Due Date': row.original.dueDate ? new Date(row.original.dueDate).toLocaleDateString() : 'N/A',
    }));
  ...
  const handleExportToPDF = () => {
    ...
    autoTable(doc, {
        head: [['Task ID', 'Title', 'Status', 'Priority', 'Assignee', 'Project', 'Start Date', 'Due Date']],
        body: tableData,
    });
        ...
        <div className="flex items-center gap-2">
            <Select 
              onValueChange={(value) => {
                ...
                table.getColumn("status")?.setFilterValue(isClearValue ? undefined : [value]);
              }}
            >
            ...
            <Select 
              onValueChange={(value) => {
                ...
                const isClearValue = value === "all";
                table.getColumn("priority")?.setFilterValue(isClearValue ? undefined : [value]);
              }}
            >
                <SelectTrigger className="w-[180px]">
