      ...
      try {
        ...
        await prisma.$executeRaw`
          INSERT INTO "DocumentVector" (
            ...
            ${idx},
            ${chunks[idx]},
            ${`[${embeddings[idx].join(',')}]`}::vector(3072),
  ...
  try {
    ...
    const results = knowledgeBaseId
      ? await prisma.$queryRawUnsafe<any[]>(`
          ...
          LIMIT $4
        `, vectorString, projectId, knowledgeBaseId, limit)
      : await prisma.$queryRawUnsafe<any[]>(`
          SELECT "id", "content", "metadata",
            1 - (embedding <=> $1::vector(3072)) as similarity
          FROM "DocumentVector"
...
export async function deleteProjectKnowledge(projectId: string) {
  try {
    ...
    console.log(`Deleted ${result.count} vectors for project ${projectId}`);
    ...
    throw new Error('Failed to delete project knowledge');
  }
}
...
export async function getKnowledgeStats(projectId: string) {
  try {
...
export async function createVectorIndexes() {
  try {
    ...
    await prisma.$executeRaw`
      CREATE INDEX IF NOT EXISTS idx_documentvector_project_id ON "DocumentVector" ("projectId")
    `;
    
    await prisma.$executeRaw`
      CREATE INDEX IF NOT EXISTS idx_documentvector_knowledge_base_id ON "DocumentVector" ("knowledgeBaseId")
    `;
    
    await prisma.$executeRaw`
      CREATE INDEX IF NOT EXISTS idx_documentvector_document_id ON "DocumentVector" ("documentId")
    `;
